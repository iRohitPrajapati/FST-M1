...
public class Chrome_Activity1 {
	...
	void activity1() throws InterruptedException {
		...
		try {
			...
			// Scroll
			driver.findElement(MobileBy.AndroidUIAutomator("UiScrollable(UiSelector().scrollable(true)).getChildByText(UiSelector().text(\"Login Form\"), \"Tab Opener\")"));
			// android.view.View[contains(@text,'Login Form')]/android.view.View
			driver.findElementByXPath("//android.view.View[contains(@text,'Login Form')]/android.view.View").click();
			// Wait
			// Invalid Credentials
			wait.until(ExpectedConditions.presenceOfElementLocated(MobileBy.id("username"))).sendKeys("adm");
			driver.findElementById("passw").sendKeys("password");
			// Login
